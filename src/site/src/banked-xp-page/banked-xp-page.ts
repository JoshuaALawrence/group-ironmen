...
const OUTFIT_PIECE_IDS: Record<string, number[]> = {
...
export class BankedXpPage extends BaseElement {
  ...
  linkedMap: Map<string, BankedItem[]>;
  enabledModifiers: EnabledModifier[];
  modifierStates: Record<string, ModifierState>;
  currentXp: number;
  currentLevel: number;
  ...
  connectedCallback(): void {
    ...
    const playerMembers = members.filter((m) => m.name !== "@SHARED");
    if (!this.selectedMember || !groupData.members.has(this.selectedMember) || this.selectedMember === "@SHARED") {
      if (playerMembers.length > 0) {
        this.selectedMember = playerMembers[0].name;
      }
    }
  ...
  handleMembersUpdated(): void {
    const members = [...groupData.members.values()];
    const playerMembers = members.filter((m) => m.name !== "@SHARED");
    if (playerMembers.length > 0 && (!groupData.members.has(this.selectedMember) || this.selectedMember === "@SHARED")) {
      this.selectedMember = playerMembers[0].name;
    }
  ...
  getFilteredActivities(itemName: string): Activity[] {
  ...
  updateDisplay(): void {
    ...
    // Update XP summary values in-place (preserves member/storage dropdowns)
    this.updateXpValues();
  ...
  /** Update XP summary text values without replacing the dropdown elements. */
  updateXpValues(): void {
    const endXp = Math.min(200000000, this.currentXp + this.bankedXpTotal);
    ...
    setText("banked-xp__val-current-xp", this.formatNumber(this.currentXp));
    setText("banked-xp__val-banked-xp", this.formatNumber(Math.floor(this.bankedXpTotal)));
    setText("banked-xp__val-end-xp", this.formatNumber(Math.floor(endXp)));
    setText("banked-xp__val-current-level", String(this.currentLevel));

    const endLevelEl = this.querySelector<HTMLElement>("#banked-xp__val-end-level");
    if (endLevelEl) {
      endLevelEl.textContent = String(this.endLevel);
      endLevelEl.style.color = this.endLevel > this.currentLevel ? "var(--green)" : "inherit";
    }
  }

  /**
   * Update item row qty/xp values in-place. Returns true if the in-place
   * update succeeded, false if the visible item set changed and a full grid
   * rebuild is needed instead.
   */
  updateItemValuesInPlace(): boolean {
    const rows = this.querySelectorAll<HTMLElement>(".banked-xp__item[data-item-id]");
    ...
    // Verify the visible item set matches the current DOM
    let visibleCount = 0;
    for (const bi of this.bankedItems) {
      const effectiveQty = getItemQty(bi, this.linkedMap, this.bankedItems, this.cascadeEnabled);
      if (effectiveQty <= 0 && bi.qty <= 0) continue;
      visibleCount++;
      if (!rowMap.has(String(bi.item.itemID))) return false;
    }
    if (visibleCount !== rows.length) return false;
    ...
    for (const bi of this.bankedItems) {
      ...
      const row = rowMap.get(String(bi.item.itemID));
      if (!row) continue;
      ...
      row.classList.toggle("ignored", bi.ignored);

      const qtyEl = row.querySelector(".banked-xp__item-qty");
      if (qtyEl) qtyEl.textContent = this.formatNumber(effectiveQty);

      const xpEl = row.querySelector(".banked-xp__item-xp");
      if (xpEl) xpEl.textContent = this.formatNumber(totalXp);

      const iconEl = row.querySelector<HTMLElement>(".banked-xp__item-icon");
      if (iconEl) iconEl.title = `Click to ${bi.ignored ? "include" : "ignore"}`;
    }
  ...
  bindDynamicEvents(): void {
    ...
    const memberSelect = this.querySelector<HTMLSelectElement>("#banked-xp__member-select");
    if (memberSelect) {
    ...
    const storageSelect = this.querySelector<HTMLSelectElement>("#banked-xp__storage-select");
    if (storageSelect) {
  ...
  bindEvents(): void {
    // Skill tabs
    this.querySelectorAll<HTMLElement>(".banked-xp__skill-tab").forEach((tab) => {
    ...
    // Cascade toggle
    const cascadeCheckbox = this.querySelector<HTMLInputElement>("#banked-xp__cascade");
    if (cascadeCheckbox) {
    ...
    const limitLevelCheckbox = this.querySelector<HTMLInputElement>("#banked-xp__limit-level");
    ...
    // XP multiplier
    const multiplierInput = this.querySelector<HTMLInputElement>("#banked-xp__multiplier");
    if (multiplierInput) {
      multiplierInput.addEventListener("change", () => {
        this.xpMultiplier = Math.max(1, Number.parseInt(multiplierInput.value, 10) || 1);
        this.saveState();
        this.recalculate();
        this.updateDisplay();
        this.bindDynamicEvents();
    ...
    // Modifier checkboxes
    this.querySelectorAll<HTMLInputElement>(".banked-xp__modifier-check").forEach((cb) => {
    ...
    this.querySelectorAll<HTMLButtonElement>(".banked-xp__modifier-piece").forEach((btn) => {
      btn.addEventListener("click", () => {
        const key = btn.dataset.modKey;
        const piece = Number.parseInt(btn.dataset.piece ?? "-1", 10);
        if (!key || piece < 0) {
          return;
        }
  ...
  bindItemEvents(): void {
    ...
    this.querySelectorAll<HTMLElement>(".banked-xp__item-icon").forEach((icon) => {
      icon.addEventListener("click", () => {
        const idx = Number.parseInt(icon.dataset.idx ?? "-1", 10);
    ...
    this.querySelectorAll<HTMLElement>(".banked-xp__item-name").forEach((nameEl) => {
      nameEl.addEventListener("click", () => {
        const idx = Number.parseInt(nameEl.dataset.idx ?? "-1", 10);
        const bi = this.bankedItems[idx];
        if (bi) {
          bi.ignored = !bi.ignored;
          this.saveItemStates();
    ...
    // Activity selects
    this.querySelectorAll<HTMLSelectElement>(".banked-xp__activity-select").forEach((select) => {
      select.addEventListener("change", () => {
        const idx = Number.parseInt(select.dataset.idx ?? "-1", 10);
        const bi = this.bankedItems[idx];
        if (bi) {
          const activities = this.getFilteredActivities(bi.item.name);
  ...
  getSavedItemStates(): Record<string, SavedItemState> {
    try {
      const raw = localStorage.getItem("banked-xp-items");
      return raw ? (JSON.parse(raw) as Record<string, SavedItemState>) : {};
    } catch {
      return {};
    }
  }
  ...
  renderXpSummary(): string {
    ...
    return `
      ...
      <div class="banked-xp__xp-row">
        ...
        <span class="banked-xp__xp-value" id="banked-xp__val-current-xp">${this.formatNumber(this.currentXp)}</span>
      </div>
      <div class="banked-xp__xp-row">
        <span class="banked-xp__xp-label">Banked XP:</span>
        <span class="banked-xp__xp-value" id="banked-xp__val-banked-xp" style="color: var(--green)">${this.formatNumber(
          Math.floor(this.bankedXpTotal)
        )}</span>
      </div>
      <div class="banked-xp__xp-row">
        <span class="banked-xp__xp-label">End XP:</span>
        <span class="banked-xp__xp-value" id="banked-xp__val-end-xp">${this.formatNumber(Math.floor(endXp))}</span>
      </div>
      <div class="banked-xp__xp-row">
        <span class="banked-xp__xp-label">Current Level:</span>
        <span class="banked-xp__xp-value" id="banked-xp__val-current-level">${this.currentLevel}</span>
      </div>
      <div class="banked-xp__xp-row">
        <span class="banked-xp__xp-label">End Level:</span>
        <span class="banked-xp__xp-value" id="banked-xp__val-end-level" style="color: ${
  ...
  renderItemRows(): string {
    return this.bankedItems
      .map((bi, idx) => {
        ...
        return `<div class="banked-xp__item ${ignoredClass}" data-item-id="${bi.item.itemID}">
