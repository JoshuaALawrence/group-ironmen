...
export type EnabledModifier = Modifier & {
  _enabledPieces?: boolean[];
};

export type BankedItem = {
  item: ExperienceItem;
  qty: number;
 ...
 * @returns {number}
 */
function getItemQtyFromBank(itemId: number, members: Map<string, GroupMemberLike>): number {
  let total = 0;
 ...
 */
export function getActivityXpRate(activity: Activity, enabledModifiers: EnabledModifier[]): number {
  ...
  for (const modifier of enabledModifiers) {
    ...
    if (modifier.type === "consumption") {
      ...
      savePercentage += modifier.savePercentage ?? 0;
      tempXp += activity.xp * (modifier.multiplier ?? 1) - activity.xp;
...
function modifierAppliesTo(modifier: EnabledModifier, activity: Activity): boolean {
...
function calculateOutfitBonusXPMultiplier(modifier: EnabledModifier): number {
...
function calculateOutfitSavePercentage(modifier: EnabledModifier): number {
 ...
 */
export function createBankedItemMap(
  skill: string,
  members: Map<string, GroupMemberLike>,
  levelLimit: number
): { bankedItems: BankedItem[]; linkedMap: Map<string, BankedItem[]> } {
  ...
  function getFilteredActivities(itemName: string): Activity[] {
  ...
  for (const targetName of linkedTargets) {
    ...
    const expItem = EXPERIENCE_ITEMS[targetName as keyof typeof EXPERIENCE_ITEMS] as ExperienceItem | undefined;
    if (!expItem) continue;
 ...
 */
export function rebuildLinkedMap(bankedItems: BankedItem[]): Map<string, BankedItem[]> {
  const linkedMap = new Map<string, BankedItem[]>();
 ...
 */
function getConsolidatedTotal(linked: Map<string, number>, goalItem: ExperienceItem, bankedItems: BankedItem[]): number {
  let total = 0;
  ...
  for (const [sourceName, sourceQty] of linked) {
    ...
    while (currentName !== goalItem.name) {
      ...
      const nextItem = EXPERIENCE_ITEMS[activity.linkedItem as keyof typeof EXPERIENCE_ITEMS] as
        | ExperienceItem
        | undefined;
      if (!nextItem) break;
 ...
 */
export function calculateBankedXpTotal(
  bankedItems: BankedItem[],
  linkedMap: Map<string, BankedItem[]>,
 ...
 */
export function calculateSecondaries(
  bankedItems: BankedItem[],
  linkedMap: Map<string, BankedItem[]>,
  cascadeEnabled: boolean
): Map<number, SecondaryNeed> {
  const needed = new Map<number, SecondaryNeed>();
  ...
  for (const bi of bankedItems) {
    ...
    const secondary = SECONDARIES[bi.selectedActivity.secondaries as keyof typeof SECONDARIES] as Secondary | undefined;
