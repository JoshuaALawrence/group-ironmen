...
type DiaryRegionCompletion = Record<DiaryTier, boolean[]>;
type DiaryCompletion = Record<string, DiaryRegionCompletion>;

export class AchievementDiary {
  static diaries: unknown;

  completion: DiaryCompletion;

  constructor(completion: DiaryCompletion) {
  ...
  static async loadDiaries(): Promise<void> {
    const response = await fetch("/data/diary_data.json");
    AchievementDiary.diaries = await response.json();
  }

  static parseDiaryData(diary_vars: number[]): DiaryCompletion {
    const createRegion = (): DiaryRegionCompletion => ({
      Easy: [],
      Medium: [],
      Hard: [],
      Elite: [],
    ...
    const result: DiaryCompletion = {
      Ardougne: createRegion(),
      Desert: createRegion(),
      Falador: createRegion(),
      Fremennik: createRegion(),
      Kandarin: createRegion(),
      Karamja: createRegion(),
      "Kourend & Kebos": createRegion(),
      "Lumbridge & Draynor": createRegion(),
      Morytania: createRegion(),
      Varrock: createRegion(),
      "Western Provinces": createRegion(),
      Wilderness: createRegion(),
    };
    ...
    return new AchievementDiary(result) as unknown as DiaryCompletion;
  }
}
