...
export class PlayerQuests extends BaseElement {
  ...
  questListElements: Map<number, HTMLElement>;
  currentQuestPointsEl: HTMLElement | null;
  searchElement: SearchElement | null;

  constructor() {
    super();
    this.questListElements = new Map();
    this.currentQuestPointsEl = null;
...
`;
  ...
  connectedCallback(): void {
    ...
    this.questListElements = new Map();
    const els = Array.from(this.querySelectorAll<HTMLElement>(".player-quests__quest"));
    for (const el of els) {
      const questId = Number.parseInt(el.getAttribute("quest-id") ?? "0", 10);
      this.questListElements.set(questId, el);
    }
    this.currentQuestPointsEl = this.querySelector<HTMLElement>(".player-quests__current-points");
    this.subscribe(`quests:${playerName}`, (quests) => this.handleUpdatedQuests(quests as Record<string, Quest>));
    this.searchElement = this.querySelector<SearchElement>("search-element");
  ...
  questSectionHtml(questsData: Record<string, { sortName?: string }>): string {
    let result = "";
...
`;
  ...
  get questPoints(): number {
    ...
    if (this.quests) {
      for (const quest of Object.values(this.quests)) {
        result += Number(quest.points);
      }
    }
    return result;
  }

  classForQuestState(questState?: string): string {
    switch (questState) {
  ...
  handleUpdatedQuests(quests: Record<string, Quest>): void {
    ...
    if (this.currentQuestPointsEl) {
      this.currentQuestPointsEl.innerHTML = String(this.questPoints);
    }
  }

  getQuestById(questId: number | string): Quest {
