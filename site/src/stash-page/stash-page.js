...
export class StashPage extends BaseElement {
  ...
  getMembers() {
    return [...groupData.members.values()];
  }
  ...
  getFilteredStashes() {
    let stashes = this.selectedTier === "All" ? STASHES : getStashesByDifficulty(this.selectedTier);
  ...
  renderTierTabs() {
    ...
    return tabs
      .map((tier) => {
        ...
        const style =
          tier !== "All" && DIFFICULTY_COLORS[tier] ? ` style="border-color:${DIFFICULTY_COLORS[tier]}"` : "";
  ...
  renderSummary() {
    ...
    const rows = DIFFICULTIES.map((d) => {
      ...
      const color = DIFFICULTY_COLORS[d] || "#ccc";
  ...
  renderStashes() {
    ...
    return stashes
      .map((stash) => {
        ...
        const diffColor = DIFFICULTY_COLORS[stash.difficulty] || "#ccc";

        const cluesHtml = clueStatuses
          .map((clue) => {
            const itemsHtml = clue.items
              .map((item) => {
                if (item.isAll && item.children) {
                ...
                <span>${child.name} (${child.have}/${child.need})</span>
                  ...
                  return `<div class="stash-page__item ${cls}">
              <img class="stash-page__item-icon" src="${iconUrl}" loading="lazy" />
              <span>${item.name} (${item.have}/${item.need})</span>
            </div>`;
                }
              })
              .join("");
  ...
  bindEvents() {
    const tierTabs = this.querySelectorAll(".stash-page__tier-tab");
    for (const tab of tierTabs) {
      this.eventListener(tab, "click", () => {
        this.selectedTier = tab.dataset.tier;
        this.render();
        this.bindEvents();
      });
    }

    const hideCheckbox = this.querySelector("#stash-page__hide-complete");
    if (hideCheckbox) {
    ...
    const searchInput = this.querySelector("#stash-page__search");
    if (searchInput) {
      searchInput.focus();
