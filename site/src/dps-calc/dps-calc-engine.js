...
import {
  ...
  ARAXXOR_IDS,
  HUEYCOATL_IDS,
} from "./dps-calc-constants.js";

import { dClawDist, burningClawSpec, burningClawDoT } from "./dps-calc-claws.js";
import { opalBolts, pearlBolts, diamondBolts, dragonstoneBolts, onyxBolts, rubyBolts } from "./dps-calc-bolts.js";

function isVampyre(attrs) {
  return (attrs || []).some(
...
export function getConflictingPrayers(key) {
...
// ── Defence Reductions ──
function getDefenceFloor(monster) {
  const id = monster.id;
...
export function applyDefenceReductions(monster, reductions) {
  ...
  reduceArclight(reductions.arclight || 0, isDemon ? [2, 20] : [1, 20]);
  reduceArclight(reductions.emberlight || 0, isDemon ? [3, 20] : [1, 20]);

  for (let i = 0; i < (reductions.tonalztic || 0); i++) {
    newSkills({ def: m.skills.def - Math.trunc(m.skills.magic / 10) });
  }

  if ((reductions.seercull || 0) > 0) {
    newSkills({ magic: m.skills.magic - reductions.seercull });
  }

  let bgsDmg = reductions.bgs || 0;
  if (bgsDmg > 0) {
    const drainStat = (k) => {
  ...
  if ((reductions.ayak || 0) > 0 && m.defensive.magic > 0) {
    m = { ...m, defensive: { ...m.defensive, magic: Math.max(0, m.defensive.magic - reductions.ayak) } };
  }
...
// ── Combat styles ──
export function getCombatStyles(category) {
  switch (category) {
    case "2h Sword":
      return [
...
export function aggregateEquipmentBonuses(equipment) {
  ...
  for (const piece of Object.values(equipment)) {
    if (!piece) continue;
    for (const key of Object.keys(totals.bonuses)) totals.bonuses[key] += piece.bonuses?.[key] || 0;
    for (const key of Object.keys(totals.offensive)) totals.offensive[key] += piece.offensive?.[key] || 0;
    for (const key of Object.keys(totals.defensive)) totals.defensive[key] += piece.defensive?.[key] || 0;
  }
  return totals;
}

function getRangedDamageType(category) {
  switch (category) {
    case "Thrown":
      return "light";
    case "Bow":
...
function calculateAttackSpeed(player, monster) {
...
function calculateEquipmentBonusesFromGear(player, monster) {
...
export class PlayerVsNPCCalc {
  constructor(player, monster, opts = {}) {
    ...
    if (player.equipment) {
      for (const v of Object.values(player.equipment)) {
  ...
  demonbaneFactor(weaponDemonbane) {
  ...
  getNPCDefenceRoll() {
    ...
    if (
      ...
      return Math.trunc((effectiveDefLevel * (defBonus + 64) * (250 + this.monster.inputs.toaInvocationLevel)) / 250);
    }
  ...
  getPlayerMaxMeleeAttackRoll() {
    ...
    if (
      ...
      TOMBS_OF_AMASCUT_MONSTER_IDS.includes(this.monster.id) &&
      (this.monster.inputs?.monsterCurrentHp || this.monster.skills?.hp) < Math.trunc(this.monster.skills.hp / 4)
    ) {
  ...
  getHitChance() {
    ...
    const ct = this.isUsingMeleeStyle() ? "melee" : this.player.style.type;
  ...
  static getNormalAccuracyRoll(atk, def) {
  ...
  static getFangAccuracyRoll(atk, def) {
    if (atk < 0) atk = Math.min(0, atk + 2);
    if (def < 0) def = Math.min(0, def + 2);
    if (atk >= 0 && def >= 0)
      return atk > def
  ...
  static getConflictionGauntletsAccuracyRoll(atk, def) {
    const s = PlayerVsNPCCalc.getNormalAccuracyRoll(atk, def);
    const d = PlayerVsNPCCalc.getFangAccuracyRoll(atk, def);
    return d / (1 + d - s);
  }

  static tbowScaling(current, magic, accuracyMode) {
  ...
  _getAttackerDist(acc, minHit, maxHit) {
      ...
      if (!spec) {
        ...
        if (this.opts.baseMonster && this.opts.defenceReductions) {
          const newReductions = {
            ...
            tonalztic: (this.opts.defenceReductions.tonalztic || 0) + 1,
          };
    ...
    if (spec && this.wearing("Abyssal dagger")) {
      ...
      dist = dist.transform((h) => new HitDistribution([new WeightedHit(1.0, [h])]).zip(secondHit), {
  ...
  _applyAttackerTransforms(dist, minHit, maxHit) {
    const mattrs = this.monster.attributes || [];
    const spec = this.opts.usingSpecialAttack;
    ...
    if (isVampyre(mattrs)) {
      ...
      const doEfaritay = (d) => (efaritay ? d.scaleDamage(11, 10) : d);
    ...
    if (this._accurateZeroApplicable) {
      dist = dist.transform((h) => HitDistribution.single(1.0, [new Hitsplat(Math.max(h.damage, 1))]), {
        transformInaccurate: false,
      });
    }
  ...
  getHtk() {
    ...
    const hp = this.monster.inputs?.monsterCurrentHp || this.monster.skills?.hp || 1;
  ...
  getSpecCost() {
    return WEAPON_SPEC_COSTS[this.player.equipment.weapon?.name];
  }
}
...
export function calculateDps({
  ...
  if (!style || !monster)
    return { dps: 0, maxHit: 0, accuracy: 0, attackSpeed: 4, attackRoll: 0, defenceRoll: 0, htk: 0, ttk: 0 };
  const player = {
    skills: {
      atk: skills.atk || 1,
      str: skills.str || 1,
